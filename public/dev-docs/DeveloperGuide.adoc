...
[[Implementation-Auth]]
=== Authentication
Authentication for Kampong App is done is 2 ways:
...
The reason for implementing social media authentication is to avoid any legal issues with regard to data usage / leakage that comes with storing sensitive user data. By implementing social media authentication, we ensure that any sensitive user data that we store is coming from an established organisation with their own set of privacy policies. (Don please rephrase this hahaha)
