...
export const getListingLocations = asyncHandler(async (req, res, next) => {
    if (req.params.listing_id) {
        ...
        const listingLocations = await db.many(
            'SELECT lil.listing_location_id, l.listing_id, lil.location_id, lo.location FROM listingsview l LEFT JOIN listinglocation lil ON l.listing_id = lil.listing_id LEFT JOIN location lo ON lil.location_id = lo.location_id WHERE l.listing_id = $1',
