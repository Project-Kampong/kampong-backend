...
export class OrganisationsRepository extends BaseRepository {
    ...
    async createOrganisation(createOrganisationData: CreateOrganisationSchema): Promise<Organisation> {
        return this.db.one('INSERT INTO organisation (${this:name}) VALUES (${this:csv}) RETURNING *', createOrganisationData);
    }

    async updateOrganisationById(updateOrganisationData: UpdateOrganisationSchema, organisationId: string): Promise<Organisation> {
